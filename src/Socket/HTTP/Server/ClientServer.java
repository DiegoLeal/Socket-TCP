...
public class ClientServer implements Runnable {
    ...
    private final String NOT_FOUND = "<html>\n" + "<head><title>404 Not Found</title></head>\n" +
            ...
            "<hr><center>ESTA PAGINA NAO EXISTE</center>\n" +
            "</body>\n" +
            "</html>";

    public ClientServer(String name , Socket client ) {
        this.name = name;
        this.client = client;
    }

    ClientServer(Socket client ) {
        this.client = client;
        name = "Client";
    }
    ...
    private void writeLog( String log ) {
        String logMsg = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss" )
                .format( Calendar.getInstance()
                .getTime() ) + " >> " + client.getInetAddress()
                .getHostAddress() + ":" + client.getPort() + " >> " + log;
        Main.logger += logMsg + "\r\n";
    ...
    private void POST_Handler( String path ) throws IOException {
        ...
        String postReply = new String( FileIOManager.readFileBytes( "http_post.html" ) )
                .replaceFirst( "<h2> Post-> </h2>" , "<h2> Post->\"" + data + "\" </h2>" );
